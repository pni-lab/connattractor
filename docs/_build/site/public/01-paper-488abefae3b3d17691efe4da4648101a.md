...
However, we do not explicitly train our ANN for a specific task. Instead, we employ an ANN architecture that shares main principles with phenomenological brain models. with its weights set empirically, with data based on the "activity flow" {cite:p}`cole2016activity; ito2017cognitive`
