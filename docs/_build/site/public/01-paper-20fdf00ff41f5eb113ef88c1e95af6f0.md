...
stimulation and self-regulation. First, we registered the frame-to-frame transitions (i.e. the vector on the 2d CHNN projection, pointing from a time-frame to the next one) in the real fMRI data for all four
...
towards a similar direction, but with a lack of a clear ghost attractor and, from most starting point, likely resulting in states that are closer to the "ghost attractor" of pain.
