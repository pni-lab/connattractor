...
The spatial patterns of the obtained attractor states exhibit high neuroscience relevance and closely resemble previously described large-scale brain systems. ({numref}`rest-validity`A). The first pair of attractors (mapped on PC1) resemble the two complementary “macro” systems described, among others, by {cite}`golland2008data` and {cite}`cioli2014differences` as well as the two "primary" brain states observed by {cite}`chen2018human`. This state-pair has previously been described as an “extrinsic” system
...
patterns with a high neurobiological relevance, resembling to sub-systems previously described as being associated for
