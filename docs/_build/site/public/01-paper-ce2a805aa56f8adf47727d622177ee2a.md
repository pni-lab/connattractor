...
However, we do not explicitly train our ANN for a specific task. Instead, we turn it to a model similar to phenological compuutational models set its weights empirically, with data based on the "activity flow" {cite:p}`cole2016activity; ito2017cognitive`
