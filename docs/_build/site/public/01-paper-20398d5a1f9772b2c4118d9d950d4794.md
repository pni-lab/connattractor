...
In study 6, 7 (ABIDE, COBRE respectively), we obtained preprocessed regional timeseries data, as shared (https://osf.io/hc4md) by {cite:t}`dadi2019benchmarking`.
Study 8 data was obtained from Alzheimer’s Disease Neuroimaging Initiative (ADNI) database (adni.loni.usc.edu) Preprocessed regional timeseries data for study 8 (ADNI) was obtained from
