...
With our approach, the topology of the functional connectome defines a position for any possible activation patterns  to a high dimensional energy landscape({numref}`concept`C) and the activity flow across regions acts as a "gravitational force" that pulls the system towards lower energy states. 
