...
Functional images were co-registered to the anatomical images with the boundary-based registration technique of FSL flirt. All resulting transformations were saved for further use. The preprocessing of functional images happened in the native image space, without resampling. Realignment-based motion-correction was performed with FSL mcflirt. The resulting six head motion estimates (3 rotations, 3 translations), their squared versions, their derivates and the squared derivates (known as the Friston-24-expansion, {cite}`friston1996movement`) was calculated and saved for nuisance correction. Additionally, head motion was summarised as framewise displacement (FD) timeseries, according to Power’s method63, to be used in data censoring and exclusion. After motion-correction, outliers (e.g. motion spikes) in timeseries data were attenuated using AFNI despike. The union of the eroded white-matter maps and ventricle masks were transformed to the native functional space and used for extracting noise-signal for anatomical CompCor correction68.


with the BASC multiscale atlas into 122 regions 
...
In this work we propose connectome-based Hopfield neural networks (CHNNs) as a model for large-scale brain dynamics. Each node in a CHNN represent a brain region. The weights of a CHNN are initilaized with a group-level 
functional connectivity matrix.

In all datasets, group-level mean functional connectivity matrices were obtained based on the regularized partial correlation between the fMRI timeseries of each pair of brain regions.
