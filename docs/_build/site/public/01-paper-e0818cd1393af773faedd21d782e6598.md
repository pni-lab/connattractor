...
We utilize an artificial neural network (ANN) as a high-level computational model of the brain. We employ an ANN architecture that is motivated by phenological models of brain dynamics.
However, we do not explicitly train our ANN for a specific task. Instead, main principles with phenomenological brain models. with its weights set empirically, with data based on the "activity flow" {cite:p}`cole2016activity; ito2017cognitive`
