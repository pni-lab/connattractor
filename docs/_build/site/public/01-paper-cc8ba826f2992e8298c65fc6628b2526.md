...
In this simplistic yet powerful framework, both spontaneous brain dynamics can be conceptualized as an intricate, high-dimensional path that meanders on the energy landscape, restricted by the "gravitational pull" of the attractors states of the system. In our model, task-induced brain activity arises from the perturbation of this path by the input (i.e. the task or stimulation). importantly, generative model for both resting state and task-related brain dynamics, offering novel perspectives on the mechanistic origins of resting state brain states and task-based activation maps.
