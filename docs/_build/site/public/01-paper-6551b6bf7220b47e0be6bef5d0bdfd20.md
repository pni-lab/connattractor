...
With our approach, the topology of the functional connectome gives rise to a high dimensional energy landscape with the activity flow across regions acting as a "gravitational force" that pulls the system towards the attractor states.
...
In the present work, we argue that state space of the system and model how it is perturbed by external stimuli (e.g. task instructions) or pathological alterations (e.g. brain disorders). 
Thus, our framework provides a generative model for both resting state and task-related brain dynamics, offering novel perspectives on the mechanistic origins of resting state brain states and task-based activation maps.
