...
However, we do not explicitly train our ANN for a specific task. Instead, we employ a neurobiologically motivated ANN architecture, which shares main principles with phenological models, with its weights set empirically, with data based on the "activity flow" {cite:p}`cole2016activity; ito2017cognitive`
