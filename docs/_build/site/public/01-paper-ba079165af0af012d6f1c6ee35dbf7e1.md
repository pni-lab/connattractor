...
Instead, it traverses extensive regions of the state space, with dynamics influenced by multiple attractor states, arising form the topology of the functional brain connectome ({numref}`concept`C). With this approach, we can map the characteristic state space of the system and model how it is perturbed by external stimuli (e.g. task instructions) or pathological alterations (e.g. brain disorders).
