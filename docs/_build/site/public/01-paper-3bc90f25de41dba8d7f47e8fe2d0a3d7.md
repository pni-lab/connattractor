...
In our further analyses, we found that the validity of the CHNN approach extends beyond the accurate reconstruction of the temporal occupancies brain states. 

CHNNs successfully reproduce fine-grained details of the bimodal distribution observed in the real resting-state fMRI data when projected onto the CHNN projection ({numref}`rest-validity`F and {numref}`attractors`E), suggesting that brain dynamics 
