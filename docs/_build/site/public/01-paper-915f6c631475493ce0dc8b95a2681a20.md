...
Hopfield networks are artificial neural networks that consist of a single layer of fully connected nodes {cite:p}`hopfield1982neural`. During the relaxation process, the activities of the nodes are updated in an iterative manner, as described by , until the network converges to a stable state, a so-called attractor state. The network's dynamics are governed by eq [](#hopfield-update).
