...
With our approach, the topology of the functional connectome gives rise to a high dimensional energy landscape of possible activation patterns ({numref}`concept`C) and the activity flow across regions acts as a "gravitational force" that pulls the system towards lower energy states. 
...
We consider the resulting state-space as model for brain dynamic activity repertoire and, similarly to network control theory, propose that changes in brain dynamics.
