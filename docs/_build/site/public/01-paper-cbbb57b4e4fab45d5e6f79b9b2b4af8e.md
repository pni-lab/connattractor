...
The table below summarizes the datasets and studies used in this analysis, including details about the study modality, analysis, sample size, mean age, gender distribution, and references. The studies include four resting state studies (study 1, study 2, study 3, and the ABIDE, NYU sample from study 6), one task-based study (study 4), a large-scale meta-analysis from Neurosynth (study 5), and two additional resting state studies focusing on specific conditions (the ADNI study on Alzheimer's Disease vs. Mild Cognitive Impairment from study 7, and the COBRE study on Schizophrenia from study 8).
