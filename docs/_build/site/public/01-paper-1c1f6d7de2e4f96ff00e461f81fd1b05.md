...
function $S(a) = tanh(a)$ and $b_i$ is the bias of unit $i$ and $\beta$ is the so-called temperature parameter. During the relaxation procedure, the activity of a region is iteratively constructed as the weighted average of the activities of all other regions, with weights defined by the connectivity between them at each iteration.
