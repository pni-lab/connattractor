...
differences (right).
**C** Simulating activity in the nucleaus accumbens (NAc) (as observed by {cite}`woo2015distinct`) reconstructs the observed non-linear flow difference between 
up- and downregulation (left).
...
**E** Visualizing meta-analyitic activation maps on the CHNN projection informs our theoretical interpretative 
framework 
