...
We obtained functional MRI data from 7 differemt sources (Table [](tab-samples)).

We included three resting state studies with healthy volunteeers (study 1, study 2, study 3), one task-based study (study 4), a large-scale meta-analysis from Neurosynth (study 5), and two additional resting state studies focusing on specific conditions (the ADNI study on Alzheimer's Disease vs. Mild Cognitive Impairment from study 7, and the COBRE study on Schizophrenia from study 8).
