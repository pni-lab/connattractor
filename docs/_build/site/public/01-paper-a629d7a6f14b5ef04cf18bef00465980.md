...
complex system dynamics  (equivalent to brain activity fluctuations in our framework) that may traverse extensive 
regions of the state space, affected by the "gravity field" (the so-called "basins") of multiple attractor states ({numref}`attractors`B).
