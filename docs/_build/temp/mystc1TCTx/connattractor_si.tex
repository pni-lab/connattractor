...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/si_orthogonality-3be51866afad7ce10d8cfa8ebed4c31d.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/si_fcann_projection-38f5e600654450896027c27da772fd02.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/expl_variance_energy-3e0c060cec3a75f05eb65951d9b8c0a2.png}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/classification_acc_s-93180f22e822a6c15450ea6be13ec54a.png}
\caption[]{\textbf{Cross-validation classification accuracy of the fcANN, when predicting the attractor state from state
...
\textbf{B} Results where simulated samples are randomly sampled from a multivariate normal distribution, with the functional connectome as the covariance matrix, and compared to the fcANN performance. See \href{https://github.com/pni-lab/connattractor/blob/master/notebooks/supplemental\_material.ipynb}{supplemental\_material.ipynb} for details.}
\label{si_state_occupancy_null_models}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/si_pain_ghost_attrac-f8c63297d0e0c56a2b4be96d12421b81.png}
\caption[]{\textbf{FcANN can reconstruct the pain ``ghost attractor''.}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/si_downreg_trajector-4f640b94503e06fb9a2cc8edfc750146.png}
\caption[]{\textbf{fcANN can reconstruct the changes in brain dynamics caused by the voluntary downregulation of pain (as contrasted to upregulation)}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/noise_robustness_wei-4a96a77072e6b086a57f3137a2bc77e9.png}
\caption[]{\textbf{Robustness of the fcANN weights to noise.}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/clinical_results_tab-8f2ab2d62409867bf46098ee000d1085.png}
...
\begin{table}
\centering
\caption[]{\textbf{MRI acquisition parameters.} TR: repetition time; TE: echo time; FA: flip angle; FOV: field of view; EPI: echo‑planar imaging; SPGR: spoiled gradient recall; SENSE/GRAPPA/ASSET: parallel imaging factors. Study 5-7 are metaanalyses or multi-center studies with varying data. Sequence parameters for these studies are available in the respective publications.}
\label{si-tab-mri}
...
Anatomical resolution / FOV & 1$\times$1$\times$1 mm\textsuperscript{3}; 256$\times$256$\times$220 mm\textsuperscript{3} & 1$\times$1$\times$1 mm\textsuperscript{3}; 256$\times$256$\times$192 mm\textsuperscript{3} & 1$\times$1$\times$1 mm\textsuperscript{3}; 256$\times$256$\times$172 & --- \\
Resting‑state EPI TR / TE / FA & 2500 ms / 35 ms / 90° & 2520 ms / 35 ms / 90° & 2500 ms / 27 ms / 81° & 2000 ms / 20 ms / --- \\
Phase enc. & COL & A\textgreater \textgreater P & A\textgreater \textgreater P & --- \\
FOV (voxels $\times$ slices) & 240$\times$240$\times$132; 40 slices & 230$\times$230$\times$132; 38 slices & 96$\times$96$\times$44; 44 slices & 64$\times$64; 42 slices \\
Slice thickness / gap / order & 3 mm / 0.3 mm / interleaved & 3 mm / 0.48 mm / interleaved & 3 mm / 0 mm / interleaved & 3 mm / --- / interleaved \\
Acceleration / fat suppression & SENSE 3$\times$ / SPIR & GRAPPA 2$\times$ / Fat sat. & ASSET 2$\times$ / Fat sat. & SENSE 1.5$\times$ / --- \\
...
We start from Eq. (\ref{steady-state-dist}), written in matrix notation: \newline
